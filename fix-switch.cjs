              ...
              <tbody>
                {[
                  ...
                  <tr key={crit.key} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                    ...
                    {data.lugares.map((l, i) => (
                      <td key={\`\${crit.key}-\${l.id}\`} className="p-4">
                        <select 
                ...
                <tr className="bg-blue-50 border-t-2 border-blue-200">
                  ...
                  {data.lugares.map((l) => (
                    <td key={\`total-\${l.id}\`} className="p-4">
              ...
              <div key={p.id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 relative hover:shadow-md transition-shadow">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2 pr-10">
                  ...
                  <div>
                    ...
                    <input type="number" min="0" className="w-full p-3 border border-slate-200 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all bg-slate-50 focus:bg-white" value={p.costoUnitario === 0 && !p.costoUnitario.toString().includes('0') ? '' : p.costoUnitario} placeholder="0" onChange={(e) => { const n = [...data.presupuesto]; n[i].costoUnitario = parseFloat(e.target.value) || 0; updateGlobalData({ presupuesto: n }); }} />
                  </div>
                  <div>
                ...
                <div className="mt-4 pt-4 border-t border-slate-100 flex justify-end">
                  <div className="text-right">
                    ...
                    <span className="ml-2 font-bold text-blue-700 text-lg">\${((p.costoUnitario || 0) * (p.cantidad || 1)).toFixed(2)}</span>
                  </div>
                </div>
              </div>
            ))}
