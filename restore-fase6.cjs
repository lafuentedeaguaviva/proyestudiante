...
const fileContent = fs.readFileSync('fix-switch.cjs', 'utf-8');

const regex = /const newSwitch = `([\s\S]*?)`;/;
const match = fileContent.match(regex);
const getPasoContentStr = match[1];

let mainContent = fs.readFileSync('src/pages/Fase6_Distribucion.jsx', 'utf-8');
const cutIndex = mainContent.indexOf('const getPasoContent = () => {');
if (cutIndex !== -1) {
  mainContent = mainContent.substring(0, cutIndex);
}

const finalCode = mainContent + getPasoContentStr + `
...
fs.writeFileSync('src/pages/Fase6_Distribucion.jsx', finalCode);
