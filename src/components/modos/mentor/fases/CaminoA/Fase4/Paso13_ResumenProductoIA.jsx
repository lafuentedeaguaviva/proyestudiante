...
import { generarResumenFase4IA } from '../../../../../../services/api';

const Paso13_ResumenProductoIA = ({ setAyudanteText, onComplete, globalData, updateGlobalData, guardando }) => {
  ...
  useEffect(() => {
    if (!resumenIAData) {
      setAyudanteText("¡Excelente! Has completado el diseño. Ahora, permite que mi Inteligencia Artificial evalúe tu producto y genere un resumen ejecutivo.");
    } else {
      setAyudanteText("Aquí está el análisis estratégico de tu producto generado por IA. Revisa que todo tenga sentido y haz clic en Finalizar.");
    }
  ...
  const generarConIA = async () => {
    if (!globalData.tipoNegocio) {
      setError("Faltan datos clave de tu producto para analizar. Completa los pasos anteriores.");
      return;
    }
    ...
    try {
      const resumen = await generarResumenFase4IA(globalData);
  ...
  return (
    <div className="animate-fade-in p-2 md:p-6" style={{ color: '#0f172a', maxWidth: '1000px', margin: '0 auto' }}>
      <h2 style={{ fontSize: '2rem', marginBottom: '2rem', textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', color: '#1e293b', fontWeight: '800' }}>
        <Sparkles color="#eab308" /> Resumen de Producto con IA
      </h2>
      ...
      {!resumenIAData && !generando && (
        <div style={{ textAlign: 'center', padding: '3rem 1rem', background: '#fefce8', borderRadius: '1rem', border: '1px dashed #fde047' }}>
          ...
          <h3 style={{ fontSize: '1.5rem', marginBottom: '1rem', color: '#854d0e' }}>Analiza tu Diseño</h3>
          <p style={{ color: '#713f12', marginBottom: '2rem', maxWidth: '600px', margin: '0 auto 2rem auto', lineHeight: '1.6' }}>
            La Inteligencia Artificial cruzará los datos de tu <strong>Producto, Características, Empaque y Demanda</strong> para estructurar el resumen ejecutivo para tu Plan.
          </p>
          <button 
      ...
      {generando && (
        <div style={{ textAlign: 'center', padding: '5rem 1rem' }}>
          ...
          <h3 style={{ color: '#ca8a04', fontSize: '1.25rem', fontWeight: 'bold' }}>Evaluando producto...</h3>
          <p style={{ color: '#713f12' }}>La IA está estructurando las características de tu propuesta.</p>
        </div>
      )}
      ...
      {resumenIAData && !generando && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={{ display: 'grid', gap: '1.5rem', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))' }}>
          ...
          <div style={cardStyle}>
            <div style={titleStyle}>📦 Concepto y Funcionalidad del Servicio/Producto</div>
            <p style={{ color: '#475569', margin: 0, lineHeight: '1.6' }}>{resumenIAData.resumen_concepto}</p>
          </div>

          <div style={cardStyle}>
            <div style={titleStyle}>🚀 Ventaja (Beneficios)</div>
            <p style={{ color: '#475569', margin: 0, lineHeight: '1.6' }}>{resumenIAData.resumen_ventaja}</p>
          </div>

          <div style={cardStyle}>
            <div style={titleStyle}>🎨 Estrategia de Empaque</div>
            <p style={{ color: '#475569', margin: 0, lineHeight: '1.6' }}>{resumenIAData.resumen_empaque}</p>
          </div>
...
export default Paso13_ResumenProductoIA;
